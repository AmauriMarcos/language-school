...
const Navbar = () => {
  ...
  const infoNav = [
    ...
    { id: 3, icon: <LocationIcon size={34} color="#FF3A3A" />, title: 'Address', text: 'Greenpoint, Brooklyn' },
  ];

  return (
    <div className="flex flex-col">
      <div
        className={`px-6 w-full flex justify-between items-center border-b border-solid border-[#e7e7e7] pb-2 md:pb-6 ${
          isScrolled ? 'fixed top-0 left-0 bg-white z-50 md:relative pt-2' : ''
        }`}
      >
      ...
      <div
        className={`hidden md:flex w-full justify-between pt-4 items-center mb-2  ${
          isScrolled
            ? 'md:fixed md:top-0 md:left-0 md:bg-white md:z-50 md:pt-4 md:shadow-md md:mb-0 px-0  sm:px-[24px]  2xl:px-[208px] py-[8px] '
            : ''
        }`}
      >
